...
typedef gtk_widget *(*gtkml_element_factory)(char **attrs);
    ...
    */
    ...
    /*if (0==g_strcmp0(name, "application")) {
        ...
        if (user_data->window==NULL && user_data->deep==1) {
            gtk_widget *gobj = factory(atts);// gtkml_window_new(atts)
        ...
        if (user_data->window==NULL) {
            // create a window
            /*
            gtk_widget *gobj = gtkml_window_new(atts);
